...
def lbADD(lbvm, *args):
    lbvm.registers[args[0]].r = lbvm.registers[args[0]].r + lbvm.registers[args[1]].r
...
def lbSET(lbvm, *args):
    print("SET")
    lbvm.registers[args[0]] = struct.unpack("<H", args[1])[0]

def lbLB(lbvm, *args):
    byte = lbvm.memory.loadByte(lbvm.registers[args[1]].r)
...
def lbOUT(lbvm, *args):
    ...
    print(chr(struct.unpack('B', args[0])[0]))

dir_of_opcodes = {0x00: (lbADD,2), 0x01: (lbCMP, 2), 0x02: (lbMOV, 2), 0x03: (lbSET, 3),
                  0x04: (lbLB, 2), 0x05: (lbSB, 2), 0x10: (lbOUT, 2)}
...
class LBVM:
    ...
    def execute(self, opcode):
        print(self.MIP.r+1)
        print(self.MIP.r+2, self.MIP.r+1 + dir_of_opcodes[self.MIP.r][1]+1)
        arg1 = self.memory.mem[self.MIP.r+1]
        arg2 = self.memory.mem[self.MIP.r+2 : self.MIP.r+1 + dir_of_opcodes[self.MIP.r][1]+1]
